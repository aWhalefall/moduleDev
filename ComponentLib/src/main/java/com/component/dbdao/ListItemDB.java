...
public final class ListItemDB {
     ...
     * @return
     */
    public String getMonthCtypeCount(String aid, String month, String year) {
        initUserId();
        //查询，得到cursor
        String sql = "SELECT *, SUM(AMOUNT)  FROM tbl_bill WHERE  A_ID = ? AND MONTH =? AND YEAR=? GROUP BY C_TYPE ORDER BY C_TYPE";
        Cursor cursor = db.rawQuery(
                sql, new String[]{aid, month, year});

        List<ListItemBO> list = new ArrayList<>();
        ListItemBO listItemBO = new ListItemBO();
        while (cursor.moveToNext()) {
            float amount = cursor.getFloat(cursor.getColumnCount() - 1);
            ListItemBO bo = listItemBO.clone();
            bo.amount = amount;
            list.add(bo);
        }
        ...
        if (list.size() > 1) {
            surplusValue = list.get(0).amount - list.get(1).amount;
        } else if (list.size() == 1) {
            surplusValue = list.get(0).amount;
        } else {
            surplusValue = 0;
        }
        cursor.close();
        return FormatUtil.getDecimalMoney2(String.valueOf(surplusValue), 2);
    }
     ...
     * @return
     */
    public String getYearCtypeCount(String aid, String year) {
        initUserId();
        //查询，得到cursor
        String sql = "SELECT *, SUM(AMOUNT)  FROM tbl_bill WHERE  A_ID = ? AND YEAR=? GROUP BY C_TYPE ORDER BY C_TYPE";
        Cursor cursor = db.rawQuery(
                sql, new String[]{aid, year});
     ...
     * @return
     */
    public String getAllYearCtypeCount(String aid) {
        initUserId();
        //查询，得到cursor
        String sql = "SELECT *, SUM(AMOUNT)  FROM tbl_bill WHERE  A_ID = ? GROUP BY C_TYPE ORDER BY C_TYPE";
        Cursor cursor = db.rawQuery(
                sql, new String[]{aid});
    ...
    public List<ListItemBO> queryBillByNoteId(String aid) {
        ...
        if (!StringUtil.isEmpty(aid)) {
            ...
            Query<ListItemBO> query = billDao.queryBuilder()
                    ...
                    .orderDesc(ListItemBODao.Properties.ADate, ListItemBODao.Properties.Ctime)
                    .build();
            // 查询结果以 List 返回
            return query.list();
        } else {
    ...
    public List<ListItemBO> queryBillByMonthAndCid(ListItemBO itemBO, int queryType) {
        initUserId();

        if (queryType == ConstanPool.QUERY_MONTH) {
            // Query 类代表了一个可以被重复执行的查询
            if (itemBO.month.startsWith("0")) {
                itemBO.month = itemBO.month.substring(1, 2);
            }
            Query<ListItemBO> query = billDao.queryBuilder()
                    .where(ListItemBODao.Properties.CType.eq(itemBO.cType),
                            ListItemBODao.Properties.AId.eq(itemBO.aId),
                            ListItemBODao.Properties.CId.eq(itemBO.cId),
                            ListItemBODao.Properties.Month.eq(itemBO.month),
                            ListItemBODao.Properties.Year.eq(itemBO.year)
                    )
                    .orderDesc(ListItemBODao.Properties.ADate)
        ...
        if (queryType == ConstanPool.QUERY_YEAR) {
            Query<ListItemBO> query = billDao.queryBuilder()
                    .where(ListItemBODao.Properties.CType.eq(itemBO.cType),
                            ListItemBODao.Properties.AId.eq(itemBO.aId),
                            ListItemBODao.Properties.CId.eq(itemBO.cId),
        ...
        if (queryType == ConstanPool.QUERY_ALL) {
    ...
    public List<ListItemBO> queryBillByDate(String date) {
        // Query 类代表了一个可以被重复执行的查询
        Query<ListItemBO> query = billDao.queryBuilder()
                .where(ListItemBODao.Properties.ADate.like("%" + date + "%")).orderAsc()
                .orderDesc(ListItemBODao.Properties.ADate)
                .build();
        return query.list();
    }
    ...
    /**
     * 根据 账本类型或者备注进行搜索 并行关系
     *
     * @param
     * @return
     */
    public List<ListItemBO> queryBillByTypeOrDescripte(String typeOrDescript) {
        // Query 类代表了一个可以被重复执行的查询
        WhereCondition whereCondition = billDao.queryBuilder()
                .or(ListItemBODao.Properties.Remark.like("%" + typeOrDescript + "%"), ListItemBODao.Properties.CName.like("%" + typeOrDescript + "%"));
        Query<ListItemBO> query = billDao.queryBuilder()
     ...
     * @return
     */
    public List<ListItemBO> queryBillByMoney(String amount) {
        // Query 类代表了一个可以被重复执行的查询
        Query<ListItemBO> query = billDao.queryBuilder()
                .where(ListItemBODao.Properties.Amount.eq(amount)).orderAsc()
                .orderDesc(ListItemBODao.Properties.ADate)
                .build();
        return query.list();
    }
    ...
    public List<ListItemBO> queryALLLocalDb() {
        Query<ListItemBO> query = billDao.queryBuilder()
                .where(ListItemBODao.Properties.LocalData.eq(ConstanPool.KEEP_TO_DB)).orderAsc()
                .orderDesc(ListItemBODao.Properties.ADate)
                .build();
        return query.list();
    ...
    public void DeleteLocalItem(ListItemBO pData) {
        billDao.delete(pData);
    }

    public void DeleteAllLocalItem(List<ListItemBO> pData) {
        billDao.deleteInTx(pData);
    }
}
